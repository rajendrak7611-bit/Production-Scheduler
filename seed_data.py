...
def seed_database():
    try:
        ...
        print("Error in create_all inside seed_database:", ex)
    ...
    try:
        try:
            ...
            db.rollback()
            print(f"Machine seeding error: {e}")
        ...
        try:
            ...
            for pfile in part_files:
                ...
                added_opn_keys = {(op.part_id, str(op.opn_no).strip(), (op.description or '').strip().upper()) for op in db.query(Operation).all()}
                for row in rows[1:]:
        ...
        try:
            ...
            db.rollback()
            print(f"Schedule seeding error: {e}")
        ...
        try:
            ...
            db.rollback()
            print(f"Tooling seeding error: {e}")
